...
#master
echo "input swarm token:"
read TOKEN
if [ $ISMASTER == "0" ]; then
	echo "creating master"
	docker-machine create -d openstack --openstack-flavor-name r620_1_4 --openstack-image-name ubuntu-trusty-150617 --openstack-net-name public --openstack-ssh-user ubuntu --swarm --swarm-master --swarm-discovery token://${TOKEN} ${AGENT_NAME}
else
	echo "creating agent"
	docker-machine create -d openstack --openstack-flavor-name r620_1_4 --openstack-image-name ubuntu-trusty-150617 --openstack-net-name public --openstack-ssh-user ubuntu --swarm --swarm-discovery token://${TOKEN} ${AGENT_NAME}
fi
...
sleep 2
echo "input user for the docke registry:"
read USER
echo "input password for the docke registry:"
read PASSWD 
...
ssh -i /home/work/nigo9731/.docker/machine/machines/${AGENT_NAME}/id_rsa -o StrictHostKeyChecking=no ubuntu@$ip << EOF
	...
	sleep 2
        sudo docker login -u ${USER} -p ${PASSWD} docker.csel.io:7777/v2/
EOF
